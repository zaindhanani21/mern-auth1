    ...
    try {
        ...
        if (!usernameRegex.test(username)) {
            return res.status(400).json({ 
                message: 'Invalid username. Must be 4-15 characters long and contain only lowercase letters, numbers, underscores (_), and dots (.). No spaces or special characters allowed!' 
            });
        }
...
router.post('/set-username', protect, async (req, res) => {
    try {
        ...
        if (!usernameRegex.test(cleanUsername)) {
            return res.status(400).json({ 
                message: 'Invalid username format. Must be 4-15 characters long and contain only lowercase letters, numbers, underscores (_), and dots (.).' 
            });
        }

        // 2. Check if user already has a social profile
        const existingProfile = await SocialProfile.findOne({ userId: req.user._id });
        if (existingProfile) {
            return res.status(400).json({ message: 'You have already activated your social profile!' });
        }

        // 3. Double check uniqueness one more time in the database
        const usernameTaken = await SocialProfile.findOne({ username: cleanUsername });
        if (usernameTaken) {
            return res.status(400).json({ message: 'Username is already taken by another user!' });
        }
...
router.post('/deactivate-social', protect, async (req, res) => {
    try {
        ...
        res.json({ message: 'Social profile deactivated and all data deleted successfully' });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
...
router.get('/search', protect, async (req, res) => {
    try {
        ...
        const foundProfile = await SocialProfile.findOne({ username: cleanUsername }).select('userId username displayName profilePicture friends');
        if (!foundProfile) {
            return res.status(404).json({ message: 'User not found!' });
        }
        ...
        if (req.user._id.toString() === foundProfile.userId.toString()) {
            ...
            const isAlreadyFriends = myProfile && myProfile.friends.some(friendId => friendId.equals(foundProfile.userId));
            if (isAlreadyFriends) {
                status = 'FRIENDS';
...
router.post('/friend-request/send', protect, async (req, res) => {
    try {
        ...
        const recipientProfile = await SocialProfile.findOne({ userId: recipientId });
        if (!recipientProfile) {
            return res.status(404).json({ message: 'Recipient social profile not found!' });
        }

        const myProfile = await SocialProfile.findOne({ userId: req.user._id });
        if (!myProfile) {
            return res.status(400).json({ message: 'Activate your social profile first!' });
        }
        ...
        res.json({ message: 'Friend request sent successfully!', requestId: newRequest._id });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
...
router.post('/friend-request/accept', protect, async (req, res) => {
    try {
        ...
        const senderProfile = await SocialProfile.findOne({ userId: request.sender });
        const recipientProfile = await SocialProfile.findOne({ userId: request.recipient });
        ...
        if (!senderProfile.friends.some(friendId => friendId.equals(recipientProfile.userId))) {
            senderProfile.friends.push(recipientProfile.userId);
            await senderProfile.save();
        }
        if (!recipientProfile.friends.some(friendId => friendId.equals(senderProfile.userId))) {
...
router.post('/friend-request/reject', protect, async (req, res) => {
    try {
        ...
        const otherPartyId = isRecipient ? request.sender.toString() : request.recipient.toString();
...
router.get('/friend-requests', protect, async (req, res) => {
    try {
        ...
        for (const request of requests) {
            const senderProfile = await SocialProfile.findOne({ userId: request.sender }).select('username displayName profilePicture');
            if (senderProfile) {
...
router.get('/friends', protect, async (req, res) => {
    try {
        ...
        for (const friendId of myProfile.friends) {
            const friendProfile = await SocialProfile.findOne({ userId: friendId }).select('username displayName profilePicture');
            if (friendProfile) {
...
router.post('/friend/remove', protect, async (req, res) => {
    try {
        ...
        myProfile.friends = myProfile.friends.filter(fId => fId.toString() !== friendId.toString());
        await myProfile.save();

        friendProfile.friends = friendProfile.friends.filter(fId => fId.toString() !== req.user._id.toString());
...
router.post('/posts', protect, async (req, res) => {
    try {
        ...
        if (cleanContent.length > 300) {
            return res.status(400).json({ message: 'Post content cannot exceed 300 characters!' });
        }

        // Validate visibility option
        const cleanVisibility = ['public', 'friends', 'private'].includes(visibility) ? visibility : 'public';
        ...
        const myProfile = await SocialProfile.findOne({ userId: req.user._id });
        const friendsList = myProfile ? myProfile.friends.map(fId => fId.toString()) : [];
...
router.get('/posts/feed', protect, async (req, res) => {
    try {
        ...
        const authorProfiles = await SocialProfile.find({ userId: { $in: authorIds } }).select('userId username displayName profilePicture');
                ...
                const populatedComments = (post.comments || []).map(comment => {
                    ...
                    return {
                        ...
                        author: {
                            ...
                            profilePicture: commenterProfile ? commenterProfile.profilePicture : null
                        }
                    };
                });
...
router.get('/posts/user/:userId', protect, async (req, res) => {
    try {
        ...
        if (isSelf) {
            ...
            if (!targetProfile) {
                return res.status(404).json({ message: 'User social profile not found!' });
            }
            
            const isFriend = targetProfile.friends.some(friendId => friendId.equals(req.user._id));
            if (isFriend) {
...
router.post('/posts/:postId/comment', protect, async (req, res) => {
    try {
        ...
        if (isSelf) {
            ...
            const authorProfile = await SocialProfile.findOne({ userId: post.author });
            if (authorProfile) {
                const isFriend = authorProfile.friends.some(friendId => friendId.equals(req.user._id));
                if (post.visibility === 'public' || (post.visibility === 'friends' && isFriend)) {
                    allowed = true;
                }
            }
        }
        ...
        if (post.author.toString() !== req.user._id.toString()) {
            const commenterProfile = await SocialProfile.findOne({ userId: req.user._id });
            const commenterName = commenterProfile ? commenterProfile.displayName : req.user.firstName;
        ...
        const populatedComment = {
            ...
            author: {
                _id: req.user._id,
                id: req.user._id,
                firstName: commenterProfile ? commenterProfile.displayName : req.user.firstName,
                lastName: '',
                username: commenterProfile ? commenterProfile.username : 'user',
                profilePicture: commenterProfile ? commenterProfile.profilePicture : req.user.profilePicture
            }
        };
...
router.get('/posts/:postId', protect, async (req, res) => {
    try {
        ...
        const authorProfile = profileMap[post.author.toString()];
        if (!authorProfile) {
            return res.status(404).json({ message: 'Post author profile not found!' });
        }
...
router.post('/posts/:postId/react', protect, async (req, res) => {
    try {
        ...
        if (!allowed) {
            return res.status(403).json({ message: 'You are not authorized to react to this post!' });
        }
     ...
     await post.save();
        ...
        if (action !== 'removed' && post.author.toString() !== req.user._id.toString()) {
            const reactorProfile = await SocialProfile.findOne({ userId: req.user._id });
            const reactorName = reactorProfile ? reactorProfile.displayName : req.user.firstName;
        ...
        const reactorProfiles = await SocialProfile.find({ userId: { $in: reactorIds } });
        const profileMap = {};
        ...
        res.json({ message: `Reaction ${action} successfully!`, reactions: populatedReactions });
    } catch (error) {
        res.status(500).json({ message: error.message });
    }
});
